...
SELECT s.shopname, c.name FROM SHOPS s inner join cats c on (s.id = c.shops_id);

SELECT s.shopname FROM SHOPS s left join cats c on (s.id = c.shops_id) where c.name like "Murzik";

SELECT s.shopname FROM SHOPS s right join cats c on (s.id = c.shops_id) where c.name like "Murzik";

SELECT s.shopname FROM shops s left join (select c.shops_id from cats c where c.name like "Murzik" or c.name like "Zuza") as ss on (s.id = ss.shops_id) where ss.shops_id is null;
...
select a.an_name,a.an_price from orders o inner join analysis a on (o.ord_an = a.an_id) where o.ord_datetime >= '2020-02-05';
